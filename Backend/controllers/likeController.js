...
export const addLike = async(req,res)=>{
    try{
        ...
        const existingLike = await Like.findOne({ UserId: userId, PostId: postId, CommentId: commentId });
        if (existingLike) {
            return res.status(400).json({ message: "You have already liked this post or comment." });
        }

        const newLike = new Like({UserId : userId, PostId : postId , CommentId : commentId}); 
        await newLike.save(); 
...
export const removeLike = async(req,res)=>{
    try{
        ...
        const like = await Like.findOneAndDelete({ UserId: userId, PostId: postId, CommentId: commentId });
        if (!like) {
